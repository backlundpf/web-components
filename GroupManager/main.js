...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    setGroup = async (groupId) => {
      var group = await fetchGroup(groupId);
      if (!group) {
        return;
      }
      ...
      await addGroupAccessMap(accessMap);

      // Fetch all users
      if (!this.applicationUsers) {
        this.applicationUsers = await fetchAllUsers();
      }

      // populate current members into search select
      this.memberSelectElement.innerHTML = "<option>Select...</option>";
    ...
    updateGroupMembers = () => {
      ...
      updateGroupMembers(this.selectedGroupId(), selectedMembers);
    };
    ...
    toggleMemberSubmit = (e) => {
      ...
      // var currentMembers = this.selectedGroup.applicationUsers.map((u) => u.id);

      // this.memberSubmitButton.toggleAttribute(
      //   "disabled",
      //   currentMembers.filter((currentMember) =>
      //     selectedMembers.includes(currentMember)
      //   ).length == 0
      // );
    };
...
async function addGroupAccessMap(accessMap) {
  console.log("adding access map:", accessMap);
}

async function removeGroupAccessMap(id) {
...
async function updateGroupMembers(groupId, users) {
